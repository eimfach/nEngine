...
constexpr std::make_index_sequence<
    std::tuple_size_v<RegisteredComponentsStorage>>
    COMPONENTS_INDEX_SEQUENCE{};
...
/*///////////////////////////////////////////
  ...
  This manager contains entities (think game objects) and each entity can build
  from different components (Like Transform, Color, Mesh etc.). The components
  are stored in a vector for each component type.
*/
class Manager {
public:
  ...
  template <typename T> void addComponent(Entity &entity, T component) {
    syncInProgress = true;
    assert((entity.hasComponentsBitmask & createComponentsMask<T>()) == 0 &&
  ...
  template <typename T> void loadIntoBuffer(const std::vector<T> &vec) {
    syncInProgress = true;
  ...
  template <typename T> T &getEntityComponent(EntityId id) {
    assert(id < entities.getWriteable().size() &&
           "ECS::Manager::getEntityComponent Out of bounds access to entities "
           "vector");
    Entity &entity = entities.getWriteable()[id];

    assert(entity.blockId < contigiousComponentsBlocks.getWriteable().size() &&
           "ECS::Manager::getEntityComponent Out of bounds access to "
           "contigiousComponentsBlocks vector");
    ContiguousComponentsBlock &block =
        contigiousComponentsBlocks.getWriteable()[entity.blockId];

    assert(componentTypeGetTupleIndex<T>() < block.offsets.size() &&
           "ECS::Manager::getEntityComponent Out of bounds access to "
           "block.offsets array");
  ...
  template <typename... T> bool hasEntityComponents(EntityId id) {
    ...
    return (
        (requested_mask & entities.getWriteable()[id].hasComponentsBitmask) ==
        requested_mask);
  }
...
private:
  ...
  std::array<BufferedVector<EntityId, std::optional<EntityId>>,
             (size_t)Groups::size>
      entityGroups{};
